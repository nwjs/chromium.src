...
void HTMLFormElement::scheduleFormSubmission(RawPtr<FormSubmission> submission)
{
    ...
    if (!targetFrame) {
        if (!LocalDOMWindow::allowPopUp(*document().frame()) && !UserGestureIndicator::utilizeUserGesture() && !document().frame()->isNodeJS())
            return;
...
void HTMLFormElement::requestAutocomplete()
{
    ...
    else if (!UserGestureIndicator::utilizeUserGesture() && !document().frame()->isNodeJS())
