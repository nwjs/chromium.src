...
bool EventMatcher::MatchNonURLCriteria(
  ...
  // https://chromium-review.googlesource.com/c/chromium/src/+/7521275
  // before: we set instance_id but didn't set has_instance_id
  // regression case: 7503-close
  if (event_info.instance_id.has_value()) {
    return event_info.instance_id.value() == GetInstanceID();
  }

  if (event_info.nw_instance_id.has_value()) {
    return event_info.nw_instance_id.value() == GetNWInstanceID();
  }
...
std::string EventMatcher::GetNWInstanceID() const {
  std::string nw_id_filter;
  if (const std::string* ptr =
          filter_->FindString("instanceId")) {
    if (base::IsStringASCII(*ptr))
      nw_id_filter = *ptr;
  }
  return nw_id_filter;
