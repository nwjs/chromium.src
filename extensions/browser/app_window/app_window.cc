...
void AppWindow::Init(const GURL& url,
  ...
  app_window_contents_->Initialize(browser_context(), creator_frame, url, GetExtension());
...
void AppWindow::UpdateAppIcon(const gfx::Image& image) {
  ...
  // as the base image.
  if (window_icon_url_.is_valid() && app_icon_image_ && !app_icon_image_->image().IsEmpty()) {
