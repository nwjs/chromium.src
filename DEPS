...
#  
#  This DEPS file corresponds to Chromium 34.0.1777.1
#  
#  
#  
vars = {
...
deps_os = {
   'win': {
      ...
      'src/third_party/nss':
      '/trunk/deps/third_party/nss@243121',
   },
   'mac': {
   ...
   'unix': {
      ...
      '/trunk/deps/third_party/openssl@236537',
      ...
      Var("git.chromium.org") + '/chromiumos/chromite.git@bbc481ce7b0712c1ffd73087c102e7844f108889',
      'src/third_party/lss':
...
deps = {
   ...
   'depot_tools':
      '/trunk/tools/depot_tools@243697',
   'src/third_party/bidichecker':
      Var("bidichecker") + '/trunk/lib@4',
   'src/third_party/WebKit':
      Var("webkit_trunk")[:-6] + '/branches/chromium/1777@164791',
   ...
   'src':
      '/branches/1777/src@243916',
   'src/tools/swarming_client':
      Var("git.chromium.org") + '/external/swarming.client.git@def5b803e9faf94885a53ffc92e3f71fe3ef0dac',
   'src/third_party/hunspell':
   ...
   'src/breakpad/src':
      Var("google-breakpad") + '/trunk/src@1262',
   'src/media/cdm/ppapi/api':
   ...
   'commit-queue':
      '/trunk/tools/commit-queue@242939',
   ...
   'build':
      '/trunk/tools/build@243911',
   'src/third_party/cld_2/src':
      Var("cld2") + '/trunk@84',
   'src/third_party/libsrtp':
      '/trunk/deps/third_party/libsrtp@214783',
   'src/third_party/ffmpeg':
      '/trunk/deps/third_party/ffmpeg@242831',
...
hooks =  [{'action': ['python', 'src/build/download_nacl_toolchains.py', '--no-arm-trusted', '--keep'], 'pattern': '.', 'name': 'nacltools'}, {'action': ['python', 'src/build/linux/install-arm-sysroot.py', '--linux-only'], 'pattern': '.', 'name': 'sysroot'}, {'action': ['python', 'src/chrome/installer/linux/sysroot_scripts/install-debian.wheezy.sysroot.py', '--linux-only', '--arch=amd64'], 'pattern': '.', 'name': 'sysroot'}, {'action': ['python', 'src/chrome/installer/linux/sysroot_scripts/install-debian.wheezy.sysroot.py', '--linux-only', '--arch=i386'], 'pattern': '.', 'name': 'sysroot'}, {'action': ['python', 'src/tools/clang/scripts/update.py', '--mac-only'], 'pattern': '.', 'name': 'clang'}, {'action': ['python', 'src/build/win/setup_cygwin_mount.py', '--win-only'], 'pattern': '.', 'name': 'cygwin'}, {'action': ['python', 'src/build/util/lastchange.py', '-o', 'src/build/util/LASTCHANGE'], 'pattern': '.', 'name': 'lastchange'}, {'action': ['python', 'src/build/util/lastchange.py', '-s', 'src/third_party/WebKit', '-o', 'src/build/util/LASTCHANGE.blink'], 'pattern': '.', 'name': 'lastchange'}, {'action': ['download_from_google_storage', '--no_resume', '--platform=win32', '--no_auth', '--bucket', 'chromium-nodejs', '-s', 'src/tools/nodejs/win/node.exe.sha1'], 'pattern': 'src/tools/nodejs/win/node.exe.sha1', 'name': 'nodejs_win'}, {'action': ['download_from_google_storage', '--no_resume', '--platform=darwin', '--no_auth', '--bucket', 'chromium-nodejs', '-s', 'src/tools/nodejs/mac/node.sha1'], 'pattern': 'src/tools/nodejs/mac/node.sha1', 'name': 'nodejs_mac'}, {'action': ['download_from_google_storage', '--no_resume', '--platform=linux*', '--no_auth', '--bucket', 'chromium-nodejs', '--directory', 'src/tools/nodejs/linux'], 'pattern': 'src/tools/nodejs/linux/*.sha1', 'name': 'nodejs_linux'}, {'action': ['download_from_google_storage', '--no_resume', '--platform=win32', '--no_auth', '--bucket', 'chromium-gn', '-s', 'src/tools/gn/bin/win/gn.exe.sha1'], 'pattern': 'src/tools/gn/bin/win/gn.exe.sha1', 'name': 'gn_win'}, {'action': ['download_from_google_storage', '--no_resume', '--platform=darwin', '--no_auth', '--bucket', 'chromium-gn', '-s', 'src/tools/gn/bin/mac/gn.sha1'], 'pattern': 'src/tools/gn/bin/mac/gn.sha1', 'name': 'gn_mac'}, {'action': ['download_from_google_storage', '--no_resume', '--platform=linux*', '--no_auth', '--bucket', 'chromium-gn', '-s', 'src/tools/gn/bin/linux/gn.sha1'], 'pattern': 'src/tools/gn/bin/linux/gn.sha1', 'name': 'gn_linux'}, {'action': ['download_from_google_storage', '--no_resume', '--platform=linux*', '--no_auth', '--bucket', 'chromium-gn', '-s', 'src/tools/gn/bin/linux/gn32.sha1'], 'pattern': 'src/tools/gn/bin/linux/gn32.sha1', 'name': 'gn_linux32'}, {'action': ['python', 'src/build/gyp_chromium'], 'pattern': '.', 'name': 'gyp'}, {'action': ['python', 'src/build/landmines.py'], 'pattern': '.', 'name': 'landmines'}] 
