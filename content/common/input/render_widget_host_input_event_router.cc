...
void RenderWidgetHostInputEventRouter::SetCursor(const ui::Cursor& cursor) {
  ...
  if (auto* cursor_manager = last_mouse_move_root_view_->GetCursorManager()) {
    for (auto it : owner_map_) {
      if (it.second) {
	TouchEmulatorImpl* impl = static_cast<TouchEmulatorImpl*>(touch_emulator);
        if (impl && impl->rfh_limit()) {
          if (static_cast<RenderFrameHostImpl*>(impl->rfh_limit())->GetView()->GetInputWeakPtr().get() != it.second.get())
            continue;
        }
