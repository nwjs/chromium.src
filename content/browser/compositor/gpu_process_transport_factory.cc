...
#elif defined(OS_MACOSX)
  if (g_force_cpu_draw)
    return std::unique_ptr<viz::SoftwareOutputDevice>(new SoftwareOutputDeviceForceCPUMac(compositor));
  else
    return std::unique_ptr<viz::SoftwareOutputDevice>(new SoftwareOutputDeviceMac(compositor));
#else
  NOTREACHED();
  return std::unique_ptr<viz::SoftwareOutputDevice>();
#endif
