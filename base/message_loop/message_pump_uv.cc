...
#define NW_HOOK_INIT(type, sym, fn) BASE_EXPORT type fn = nullptr;
#define NODE_HOOK_INIT(type, sym, fn) BASE_EXPORT type fn = nullptr;
...
MessagePumpUV::MessagePumpUV()
  ...
  g_msg_pump_ctor_fn(&wakeup_event_, worker_support);
}
...
void MessagePumpUV::Run(Delegate* delegate) {
  ...
  // should be called before g_msg_pump_nest_leave_fn() or there will
  // be heap overrun in nest loop case like NWJS#5742
