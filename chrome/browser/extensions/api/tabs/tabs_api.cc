...
bool UpdateWindowBoundsFromParams(const api::windows::Update::Params& params,
                                  gfx::Rect& window_bounds, BrowserWindowInterface* b, bool& set_pos_only) {
  ...
  gfx::Size max_size = BrowserView::GetBrowserViewForBrowser(b)->GetMaximumSize();
  ...
  if (set_min_size)
    BrowserView::GetBrowserViewForBrowser(b)->SetMinimumSize(min_size);
  if (set_max_size)
    BrowserView::GetBrowserViewForBrowser(b)->SetMaximumSize(max_size);
...
ExtensionFunction::ResponseAction WindowsCreateFunction::Run() {
  ...
  if (create_data_) {
    ...
    if (create_data_->min_width) {
      min_width = *create_data_->min_width;
      if (window_bounds.width())
        window_bounds.set_width(std::max(min_width, window_bounds.width()));
    }
    if (create_data_->max_width) {
      max_width = *create_data_->max_width;
      window_bounds.set_width(std::min(max_width, window_bounds.width()));
    }
    if (create_data_->min_height) {
      min_height = *create_data_->min_height;
      if (window_bounds.height())
        window_bounds.set_height(std::max(min_height, window_bounds.height()));
    }
    if (create_data_->max_height) {
      max_height = *create_data_->max_height;
    ...
    if (create_data_->kiosk)
      kiosk = *create_data_->kiosk;
    if (create_data_->alpha_enabled)
      transparent = *create_data_->alpha_enabled;
    if (create_data_->always_on_top)
      always_on_top = *create_data_->always_on_top;
    if (create_data_->all_visible)
      all_visible = *create_data_->all_visible;
    if (create_data_->resizable)
      resizable = *create_data_->resizable;
    if (create_data_->show_in_taskbar)
      show_in_taskbar = *create_data_->show_in_taskbar;
    if (create_data_->title)
  ...
  if (create_data_ && create_data_->icon) {
    base::ScopedAllowBlocking allow_io;
    gfx::Image app_icon;
    nw::Package* package = nw::package();
    if (nw::GetPackageImage(package,
                            base::FilePath::FromUTF8Unsafe(*create_data_->icon),
                            &app_icon)) {
  ...
  ExtensionFunction::ResponseAction response =
...
ExtensionFunction::ResponseAction WindowsCreateFunction::OnBrowserWindowCreated(
    BrowserWindowInterface* new_window,
  ...
  BrowserView::GetBrowserViewForBrowser(b)->SetMinimumSize(gfx::Size(min_width, min_height));
  BrowserView::GetBrowserViewForBrowser(b)->SetMaximumSize(gfx::Size(max_width, max_height));

#if defined(OS_LINUX) || defined(OS_WIN)
  if (!resizable)
    BrowserView::GetBrowserViewForBrowser(b)->SetResizable(false);
#endif
  ...
  if (focused) {
    ...
    BrowserView::GetBrowserViewForBrowser(b)->Hide();
  }
...
void WindowsCreateFunction::OnFinishedFirstNavigationOrClosed(
  ...
  Respond(WithArguments(ExtensionTabUtil::CreateWindowValueForExtension(
      *browser, extension, WindowController::kPopulateTabs, context)));
}
...
ExtensionFunction::ResponseAction WindowsUpdateFunction::Run() {
  ...
  const bool set_window_bounds =
      UpdateWindowBoundsFromParams(*params, window_bounds, browser, set_pos_only);
  ...
  if (set_client_bounds) {
    ...
    BrowserView::GetBrowserViewForBrowser(browser)->SetBounds(win_bounds);
  }
...
void WindowsUpdateFunction::UpdateWindowState(
  ...
  if (params.update_info.position &&
    ...
    BrowserView::GetBrowserViewForBrowser(browser)
        ->browser_widget()->CenterWindow(window_bounds.size());
  if (params.update_info.position &&
      *params.update_info.position == "mouse") {
    BrowserWidget* browser_frame =
  ...
  if (params.update_info.resizable) {
    BrowserView::GetBrowserViewForBrowser(browser)->SetResizable(*params.update_info.resizable);
  }
  if (params.update_info.all_visible) {
    BrowserView::GetBrowserViewForBrowser(browser)->SetAllVisible(*params.update_info.all_visible);
  }
  if (params.update_info.always_on_top) {
    BrowserView::GetBrowserViewForBrowser(browser)->SetZOrderLevel(*params.update_info.always_on_top ? ui::ZOrderLevel::kFloatingWindow : ui::ZOrderLevel::kNormal);
  }
  if (params.update_info.show_in_taskbar) {
    BrowserView::GetBrowserViewForBrowser(browser)->SetShowInTaskbar(*params.update_info.show_in_taskbar);
  }
