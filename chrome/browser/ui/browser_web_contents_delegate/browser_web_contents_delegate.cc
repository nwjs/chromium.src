...
bool BrowserWebContentsDelegate::HandleKeyboardEvent(
  ...
  if (event.windows_key_code == ui::VKEY_ESCAPE) {
    auto* controller = exclusive_access_manager_->pointer_lock_controller();
    bool is_keydown =
...
void BrowserWebContentsDelegate::LoadingStateChanged(
  ...
  if (source->IsLoading()) {
    nwstatus = "loading";
    browser->last_to_different_document_ = should_show_loading_ui ? 1 : 0;
    if (!should_show_loading_ui)  // NWJS#5001
      return;
  } else {
    if (browser->last_to_different_document_ == 0)
      return;
    nwstatus = "loaded";
  }
  ...
  if (!tabs_window_api)
    return;
  extensions::TabsEventRouter* tabs_event_router =
      tabs_window_api->tabs_event_router();
...
#if 0
  // nw2: disable hung renderer dialog #7339
