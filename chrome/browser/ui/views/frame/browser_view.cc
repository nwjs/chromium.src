...
BrowserView::BrowserView(BrowserWindowInterface* browser)
  ...
  resizable_ = browser_->GetBrowserForMigrationOnly()->initial_resizable();
#if defined(OS_LINUX) || defined(OS_WIN)
  if (!resizable_) {
    gfx::Size size =
        BrowserInitState::From(browser_.get())->override_bounds().size();
  ...
  if (browser_->GetBrowserForMigrationOnly()->is_transparent())
...
bool BrowserView::NWCanClose(bool user_force) const {
  return browser_->GetBrowserForMigrationOnly()->NWCanClose(user_force);
}

void BrowserView::UpdateDraggableRegions(
   const std::vector<blink::mojom::DraggableRegionPtr>& regions) {
  // Draggable region is not supported for non-frameless window.
  if (!browser_->GetBrowserForMigrationOnly()->is_frameless())
    return;
...
void BrowserView::SetAllVisible(bool visible) {
  browser_widget_->SetVisibleOnAllWorkspaces(visible);
}
...
void BrowserView::SetResizable(bool resizable) {
  ...
  browser_widget_->non_client_view()->ResetWindowControls();
  browser_widget_->non_client_view()->LayoutImmediately();
}
...
ui::ImageModel BrowserView::GetWindowIcon() {
  gfx::Image icon_override = browser()->GetBrowserForMigrationOnly()->icon_override();
  ...
  if (browser_->GetType() == BrowserWindowInterface::Type::TYPE_DEVTOOLS) {
    ...
    if (devtools_window) {
      ...
      BrowserWindowInterface* bwi2 =
          GlobalBrowserCollection::GetInstance()->FindBrowserWithTab(
              inspected_contents);
      Browser* browser = bwi2 ? bwi2->GetBrowserForMigrationOnly() : nullptr;
      if (browser && !browser->icon_override().IsEmpty())
  ...
  if (chrome::SavedBoundsAreContentBounds(browser_.get())) {
    ...
    if (show_state == ui::mojom::WindowShowState::kMaximized ||
      ...
      gfx::Rect window_bounds =
          browser_widget_->non_client_view()->GetWindowBoundsForClientBounds(client_bounds);
...
void BrowserView::NativeWindowChanged() {
  extensions::TabsWindowsAPI* tabs_window_api =
    extensions::TabsWindowsAPI::Get(browser_->GetProfile());
  tabs_window_api->windows_event_router()->
    OnWindowChanged(browser_ ?
                    extensions::BrowserExtensionWindowController::From(browser_.get()) : nullptr);
}
