...
void BrowserViewLayoutImplOld::Layout(views::View* browser_view) {
  ...
  if (views().menu_bar) {
    views().menu_bar->SetBounds(0, available_bounds.y(), views().browser_view->width(), kMenuHeight);
...
gfx::Point BrowserViewLayoutImplOld::GetDialogPosition(
  ...
  const int top = rect.y() + rect.height() / 2 - dialog_size.height() / 2;
  const int middle_x =
      leading_x + layout_result.contents_container_bounds.width() / 2;
  return gfx::Point(middle_x - dialog_size.width() / 2, top);
}
