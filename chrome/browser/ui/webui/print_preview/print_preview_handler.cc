...
static base::NoDestructor<base::DictValue> g_nw_print_options;
...
void NWPrintSetOptions(const base::DictValue* dict, WebContents* web_contents) {
...
void PrintPreviewHandler::HandleGetPreview(const base::ListValue& args) {
  ...
  base::DictValue* dict = &(*g_nw_print_options);
  if (dict) {
    base::DictValue* media_size_value = dict->FindDict(printing::kSettingMediaSize);
    base::DictValue* custom_margins = dict->FindDict(printing::kSettingMarginsCustom);
...
void PrintPreviewHandler::HandleDoPrint(const base::ListValue& args) {
  ...
  base::DictValue* dict = &(*g_nw_print_options);
  if (dict) {
    base::ListValue* page_range_array = dict->FindList(printing::kSettingPageRange);
    bool changed = false;
