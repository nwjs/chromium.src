...
void ChromeCrashReporterClient::GetProductInfo(ProductInfo* product_info) {
...
#else
  // nw: use the product name configured for this build, if any.
  product_name = product_name_.empty() ? "Chrome_Linux" : product_name_.c_str();
#endif  // defined(ADDRESS_SANITIZER)
#elif BUILDFLAG(IS_MAC)
  // nw: use the product name configured for this build, if any.
  product_name = product_name_.empty() ? "Chrome_Mac" : product_name_.c_str();
  ...
  // nw: use the product version configured for this build, if any.
  const char* product_version =
      product_version_.empty() ? UpdaterVersion() : product_version_.c_str();
  *product_info =
      ProductInfo(product_name, product_version,
